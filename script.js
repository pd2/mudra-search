    ...
    var endTurn = function () {
      ...
      for (var i = 0, len = wordList.length; i < len; i++) {
        ...
        if (wordList[i] === curWord) {
          ...
          status.innerHTML = `${curClue} mudra represents ${curWord} `;
...
function CreateGame() {
  ...
  let arr = Array.from({length: mudra_list.length}, (x, i) => i);
  let rnd = shuffle(arr);
  rnd.length = num_objects;
  
  let words = rnd.map(i => mudra_list[i].object);
  let clues = rnd.map(i => mudra_list[i].name);
...
//  var clues = ['1 save', '2 earn', '3 invest', '4 retire', '5 account', '6 money', '7 credit', '8 debt', '9 assets', '10 loan', '11 interest', '12 accrual', '13 economy', '14 sharing', '15 savings', '16 budget', '17 capital', '18 lateral', '19 bond', '20 market', '21 value', '22 index'];  
  ...
  for(var i=0; i<num_objects; i++) {
    ...
    Tile.style.width = `${100/num_objects}%`;
    Tile.innerHTML = `<img src="${cdn_path}/${clues[i]}.jpg">`;
  }
...
function ShareIt() {
  ...
  status.innerHTML = `Congrats. You searched for ${global_clues[0]} Bharatanatyam mudra in ${Math.round(elapsedTime/1000)} sec`
  ...
  navigator.clipboard.writeText(copyText);
   ...
   if (navigator.canShare) {
    navigator.share({
      title: 'Share results',
      text: `I solved #Mudra search in a letter grid game to learn about ${global_clues[0]} Bharatanatyam mudra in ${Math.round(elapsedTime/1000)} sec at ${linkURL}`,
      // url: linkURL,
    })
